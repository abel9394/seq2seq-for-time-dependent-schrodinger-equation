    ...
    def next_batch(self, batch_size, shuffle=True):
        ...
        if start + batch_size > self._num_examples:
            ...
            return np.concatenate((enc_inps_rest_part, enc_inps_new_part), axis=0), \
                   ...
                   np.expand_dims(np.concatenate((dec_outs_rest_part, dec_outs_new_part), axis=0)[:,:,0],2)
        else:
            self._index_in_epoch += batch_size
            end = self._index_in_epoch
            return self._enc_inps[start:end], self._dec_inps[start:end], np.expand_dims(self._dec_outs[start:end,:,0],2)
...
def read_data_sets(data_path, s2s, n_steps,
    ...
    ntest = test_size
    nval = val_size
    train_size = len(data)-ntest-nval
    test_size = len(data)-ntest
